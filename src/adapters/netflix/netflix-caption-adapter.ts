...
export class NetflixCaptionAdapter {
  ...
  private setupNavigationListeners() {
    ...
    // Netflix is an SPA and normally navigates with pushState/replaceState.
    // Patch them once so the adapter can reset its caption state immediately.
  ...
  private getOverlay(): HTMLElement {
    ...
    if (!overlay) {
      ...
      overlay.style.width = 'min(90vw, 1000px)';
      overlay.style.maxWidth = '90vw';
      overlay.style.boxSizing = 'border-box';
    }

    // Keep the layer inside the fullscreen tree when Netflix enters fullscreen,
    // otherwise attach directly to body to escape player clipping/stacking.
  ...
  private setupOverlayPositionListeners() {
    ...
    const reposition = () => {
      ...
      if (overlay) this.positionOverlay(overlay);
    };
  ...
  private getNativeSubtitleElement(): HTMLElement | null {
    const elements = this.getNativeSubtitleElements();
    return (
      elements.find((element) => element.matches('.player-timedtext-text-container')) ||
      elements.find((element) =>
        element.matches('[data-uia="player-timedtext"], [data-uia="watch-video--timed-text"]'),
      ) ||
      elements[0] ||
      null
    );
  ...
  private hideNativeSubtitles() {
    for (const element of this.getNativeSubtitleElements()) {
      if (!this.hiddenNativeSubtitleStyles.has(element)) {
        this.hiddenNativeSubtitleStyles.set(element, element.style.visibility);
      }
  ...
  private restoreNativeSubtitles() {
    for (const [element, visibility] of this.hiddenNativeSubtitleStyles) {
      element.style.visibility = visibility;
    }
    this.hiddenNativeSubtitleStyles.clear();
  }
  ...
  private startObserver() {
    ...
    // Observe a stable page root instead of the caption node itself. Netflix
    // replaces the caption node on almost every subtitle change.
    const targetNode =
  ...
  private processCaptions() {
    ...
    // Reading captions synchronously is intentional. Netflix can replace the
    // timed-text node between two timer ticks; processing now avoids a blank
    // frame and still remains cheap because unchanged text is deduplicated.
  ...
  private async fetchAndRenderOverlay(text: string, generation: number) {
    ...
    try {
      ...
      const translatedText =
        response?.segments?.find((segment) => segment.id === 'nf-overlay')?.translatedText ||
  ...
  private renderOverlay(originalText: string, translatedText: string) {
    ...
    const createLine = (text: string, color: string, bold: boolean, fontSize: string) => {
    ...
    if (this.displayMode === 'immersive') {
      translatedLines.forEach((line) =>
        container.appendChild(createLine(line, this.subtitleTranslatedColor, true, translatedFontSize)),
      );
      ...
      originalLines.forEach((line) =>
        container.appendChild(createLine(line, this.subtitleOriginalColor, false, originalFontSize)),
      );
    } else {
      originalLines.forEach((line) =>
        container.appendChild(createLine(line, this.subtitleOriginalColor, false, originalFontSize)),
